...
namespace FilesUrl.Providers
{
    public class ApplicationOAuthProvider : OAuthAuthorizationServerProvider
    {
        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            var allowLogin = ApplicationDbContext.Create().SettingParams.Where(s => s.Param == "allowLogin").Select(s => s.Value).FirstOrDefault();
            if (user.UserName != "student" && !(int.Parse(allowLogin) > 0))
            {
                context.SetError("error", "登录受限。");
                return;
            }

            // 记录登录信息
            DbLogsHelper.Create().CreateLog("Login", user.UserName, "");
